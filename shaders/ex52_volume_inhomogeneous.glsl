//
// Inhomogenous volume light transport Monte-Carlo (Delta-tracking)
//
...
// Examples
// 1. Only scattering volume without scene geometry
//   kSigmaAbs: 0.0
//   kSigmaSca: 4.0
//   kSceneType: 1
//
// 2. Radiance absorbing volume within diffuse room
//   kSigmaAbs: 4.0
//   kSigmaSca: 0.0
//   kSceneType: 0
//
...
const float kSigmaSca = 4.0;
const float kSigmaAtt = kSigmaAbs + kSigmaSca;

// camera
const float kYfov = 39.0 * M_PI / 180.0;
const vec3  kCameraP = vec3(0.0, 0.0, 4.0);
const vec3  kLookatP = vec3(0.0);

// scene geometry
const int kSceneType = 1;

const vec3 kVolumeBboxMin = vec3(-1.0);
const vec3 kVolumeBboxMax = vec3(+1.0);
...
// TODO: can't prove/disprove but it might be indirect bounce is too strong (i.e. brdf bug?)
//       for now, clamp indirect bounce as a surgery.
const float kAdhocDiffuseIndirectClamp = 10.0;
...
// lighting
const PointLight kLight = PointLight(
    vec3(0.0, 0.0, 0.0),
    vec3(1.0) * M_PI * 1.0);
...
bool Bbox_intersect_v2(vec3 bb_min, vec3 bb_max, Ray ray, out float tmin, out float tmax) {
  // Interesect to six planes
  vec3 t0 = (bb_min - ray.o) / ray.d;  // "negative" planes
  vec3 t1 = (bb_max - ray.o) / ray.d;  // "positive" planes
  ...
  bool hit = (t_in < t_out) && 0.0 < t_out && ( // half-line crosses box (i.e. ray without tmax)
...
bool Scene_intersect(inout Ray ray, bool any_hit, out Intersection isect) {
  ...
  if (kSceneType == 0) {
    ...
    {
      // bottom
      Box prim; prim.c = vec3(0.0, -1.0, 0.0); prim.s = vec3(1.0, w, 1.0);
      PRIMITIVE(Box, prim);
    }
    {
      // top
    ...
    {
      // right
      Box prim; prim.c = vec3(+1.0, 0.0, 0.0); prim.s = vec3(w, 1.0, 1.0);
      PRIMITIVE(Box, prim);
    }
    ...
    {
      // back
      Box prim; prim.c = vec3(0.0, 0.0, -1.0); prim.s = vec3(1.0, 1.0, w);
      PRIMITIVE(Box, prim);
    }
  }
...
void Scene_evaluateLight(Intersection isect, out vec3 wi, out vec3 Le) {
  evaluatePointLight(kLight, isect, wi, Le);
}
...
float Scene_volumeSigmaRatio(vec3 p) { // [-1, 1]^3 -> [0, 1]
  float density = Noise_valueNoise(4.0 * p);

  // Clip lower value and rescale
  density = smoothstep(0.5, 1.0, density);

  // Crop out center where it's too close to point light
  float fac = smoothstep(0.0, 1.0, (length(p) - 0.2) / 0.2);
  return fac * density;
}

// Return throughput-update irregardless of volume scattering event
float Scene_sampleVolumeInhomogeneous(Ray ray, out Intersection isect, out bool hit_vol) {
  // Monte carlo evaluation of
  //   (\int_{t < tmax} Tr(t) s_sca(t) L_phase) + Tr(tmax) L_surf
  //   where
  //     Tr(t) = exp(- \int_{t' < t} s(t'))

  // Check volume box intersection
  float tmin, tmax;
  if (!Bbox_intersect_v2(kVolumeBboxMin, kVolumeBboxMax, ray, /*out*/ tmin, tmax)) {
    hit_vol = false;
    return 1.0;
  }

  // Sample from density "s(t) Tr(t)" clamped at t = tmax by Delta-Tracking
  float t = tmin;
  uint seed = 0u;
  while (true) {
    seed += 1u;

    // 1. Sample from "s exp(- s x)"
    float u1 = hash41(vec4(ray.d, hash41(vec4(ray.o, float(hash21u(uvec2(uint(iFrame), seed)))))));
    float x = - log(1.0 - u1) / kSigmaAtt;
    t += x;

    // 1.1. Sampled clamped delta part (right term)
    if (tmax <= t) {
      hit_vol = false;
      return 1.0;
    }

    // 2. Accept with prob. s(t) = sigmRatio * kSigmaAtt
    vec3 p = ray.o + t * ray.d;
    float sigmaRatio = Scene_volumeSigmaRatio(p);
...
float Scene_evaluateTransmission(Ray ray) {
  const bool kUseRatioTracking = true;
  if (!kUseRatioTracking) {
    //
    // Monte carlo evaluation by
    //   Tr(tmax) = exp(- \int_{t < tmax} s(t))
    //            = (\int_{t < tmax} Tr(t) "0") + Tr(tmax) "1"
    //
    Intersection isect;
    bool hit_vol;
    Scene_sampleVolumeInhomogeneous(ray, /*out*/ isect, hit_vol);
    return float(!hit_vol);
  }

  //
  // By Ratio-tracking (it looks magical but you can prove E[Tr] = Tr(tmax))
  //
  ...
  float Tr = 1.0;
  float t = tmin;
  uint seed = 0x1048ab18u; // Use different seed from Delta-tracking in `Scene_sampleVolumeInhomogeneous`
  while (true) {
    seed += 1u;
    ...
    // 2.2 Update by "1.0 - sigmRatio * kSigmaAtt"
    vec3 p = ray.o + t * ray.d;
    float sigmaRatio = Scene_volumeSigmaRatio(p);
    Tr *= 1.0 - sigmaRatio;
  }
...
vec3 Volume_Li(Ray ray) {
  ...
  for (int i = 0; i <= kMaxVolumePathLength; i++) {
    ...
    bool hit_vol;
    throughput *= Scene_sampleVolumeInhomogeneous(ray, /*out*/ isect_vol, hit_vol);
