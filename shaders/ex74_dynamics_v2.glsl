...
//
// - Only evolves limited time (sort of vector field visualization)
//
// TODO:
// - draw by line (anti alias etc...)
//

/*
// length of trajectory
%%EXEC: os.environ['N1'] = '60'%%

// number of trajectories
%%EXEC: os.environ['N2'] = '200'%%
*/
...
plugins:
  ...
  - type: rasterscript
    params:
      exec: |
        import numpy as np
        indices =  np.uint32(np.arange(%%ENV:N1:%% * %%ENV:N2:%%))
        RESULT = [bytes(), bytes(indices)]
      primitive: GL_POINTS
      # [ Line (not quite right yet )]
      #exec: |
      #  import numpy as np
      #  a =  np.uint32(np.arange(%%ENV:N1:%% * %%ENV:N2:%%))
      #  indices = (np.stack([a, a + 1]).T).reshape(-1)
      #  RESULT = [bytes(), bytes(indices)]
      #primitive: GL_LINES
  ...
  # [ Uniform ]
  - type: uniform
    params: { name: U_mu, default: 1, min: 0, max: 4}
...
const vec3  kCameraP = vec3(0.0, 0.0, 1.0) * 15.0;
...
ivec2 fromIndex(int idx, ivec2 size) {
  ...
  void main() {
    ivec2 coord = fromIndex(gl_VertexID, kSize);
