//
// RasterscriptPlugin instancing test
//
  ...
  - type: rasterscript
    params:
      exec: |
        ...
        RESULT = ex00.example('hedron20', num_subdiv=1, smooth=True)
      instance_count: 512
...
//
  ...
  vec3 getInstancePosition(uint id) {
    vec2 u = hash12(float(id));
    vec3 p;
    float pdf;
  ...
  void main() {
    vec3 p = getInstancePosition(gl_InstanceID);
    Interp_position = p + 0.04 * Vertex_position;
